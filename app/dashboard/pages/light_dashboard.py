...
st.set_page_config(page_title="조도 모니터링", layout="wide", initial_sidebar_state="collapsed")
...
def load_data():
    try:
        ...
        query = """
            SELECT timestamp, ex_illuminance 
...
def main():
    ...
    current_val = df['ex_illuminance'].iloc[0]
    prev_val = df['ex_illuminance'].iloc[1]
    avg_val = df['ex_illuminance'].mean()
    ...
    fig = px.line(
        ...
        x="timestamp",
        y="ex_illuminance",
        labels={"ex_illuminance": "조도 (lux)", "timestamp": "시간"}
    )
    
    fig.update_traces(
        line_color="#FFD700",
        line_width=1.5,
        hovertemplate="시간: %{x}<br>조도: %{y:.1f} lux<extra></extra>"
    )
